...
- **In-page addressing guard** for dialog-less irreversible controls. When the button opens no dialog (nothing for `confirm` to read), `click --ref REF --require-ancestor-text "Recipient Name"` walks up to the target's nearest self-contained card ancestor (`article`, `li`, `[role=listitem]`, `dialog`, `section`, ... — or the outermost block inside the enclosing landmark) and **refuses to click** unless that card's text contains the value: typed `ASSERTION_FAILED` (exit 3), no input dispatched. This is the guard against duplicate labels — a sidebar of "Connect" buttons for *other* people fails closed instead of being clicked. The result reports `ancestorGuard: { matched, card }`.
