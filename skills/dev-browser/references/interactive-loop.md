...
**The same logical action can have different UI paths on the same site.** On LinkedIn, for example, some profiles expose "Connect" as a primary button while others bury it in the "..." overflow menu that must be opened first — and the sidebar repeats the same label for *other* people. Never assume the recipe that worked on one page works on the next: perceive first, disambiguate with `--within`/`--near`, and `confirm --expect` the recipient before anything irreversible.
...
- **Confirmation tokens** for irreversible actions. `confirm --ref REF --expect "Recipient"` returns a token that is daemon-scoped, single-use, bound to the exact page/document/target/URL, and expires in 30 seconds. Consume it on the final action with `--from-state STATE --confirm-token TOKEN`. An invalid/expired/reused/out-of-scope token returns `CONFIRMATION_INVALID` (exit status 8) — the action does **not** fire. The confirmation text is read from **the dialog containing the target ref** (real pages keep unrelated `[role=dialog]` surfaces mounted permanently — LinkedIn's messaging overlay, for one — so "the last visible dialog" is often the wrong surface); a target with no dialog ancestor falls back to the last visible dialog, then body. On mismatch the error reports the scope it read and a bounded sample of the observed text, so a wrong-surface read is a one-look diagnosis.
- **In-page addressing guard** for dialog-less irreversible controls. When the button opens no dialog (nothing for `confirm` to read), `click --ref REF --require-ancestor-text "Recipient Name"` collects the target's self-contained card ancestors (`article`, `li`, `[role=listitem]`, `form`, `section`, `dialog`, ...) nearest-first and **refuses to click** unless one of them contains the value: typed `ASSERTION_FAILED` (exit 3), no input dispatched. It escalates outward through enclosing cards — a composer's `form` rarely holds the recipient shown in the same dialog's header — but **never past a dialog ancestor or the enclosing landmark**: text outside a modal can never vouch for a target inside it. This is the guard against duplicate labels — a sidebar of "Connect" buttons for *other* people fails closed instead of being clicked. The result reports `ancestorGuard: { matched, card }`; failures list every card examined.
