...
- Added direct semantic targets for trusted actions (`--role`, `--name`, `--within`, `--near`, `--frame`, repeated `--state`) with fresh resolution, fail-closed ambiguity handling, compact recovery candidates, and semantic retry commands in stale/target errors.
- Added protocol-v2 `batch` execution (up to 20 guarded steps under one page lock), standalone wait steps, first-error exit propagation, and final `delta`, `tree`, or `none` observation modes.
- Added compound shortcuts: `type --press KEY`, repeatable `type --fill REF=TEXT`, `navigate --observe [SCOPE]`, and `click --then-text SCOPE`.
- Removed fixed post-input latency floors, replaced repeated action perceptions and target-resolution CDP round trips with bounded in-page passes, made typed waits reliable on large pages, batched frame geometry, reused screenshots, enabled per-page concurrency, and pooled isolated warm QuickJS runtimes.
- Added a fast-path agent guide: discover with `capabilities --compact`, use semantic actions and batches for reversible work, read with scoped `text`, reserve screenshots for spatial/irreversible decisions, and retain the full guarded loop for consequential actions.
- Compact responses are now the default: CLI JSON is single-line (`--pretty` restores indentation), `observe` omits `elements` unless `--elements` or `--verbose` is requested, actions return bounded deltas without diagnostic journals outside verbose mode, and `find` returns at most three compact matches without a tree (up to five contextual candidates when ambiguous). Coordinates are rounded deterministically, routine warnings are suppressed, and protocol v1 plus the full `--verbose` shape remain compatible.
