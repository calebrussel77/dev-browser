...
- Fixed `confirm` verifying the wrong surface when the page keeps unrelated dialogs mounted. The confirmation text is now read from the dialog that contains the target ref, on both token issuance and consumption (previously "the last visible `[role=dialog]` on the page" — on LinkedIn that is the permanently mounted messaging overlay, so the invitation modal's `confirm` refused every value including the target's own label). Targets without a dialog ancestor keep the historical fallback. On mismatch, `CONFIRMATION_INVALID` now reports which scope was read and a bounded sample of the observed text.
- `click --require-ancestor-text` now escalates through enclosing card ancestors (nearest first) instead of judging only the closest one — a composer's `form` rarely contains the recipient shown in the same dialog's header — and treats a dialog ancestor as a hard boundary: text outside a modal can never vouch for a target inside it. Failures list every card examined.
- Oversized error `details` now degrade to a `{ truncated: true }` stub instead of turning into a Zod validation failure that replaced the real error (observed live: a `WAIT_TIMEOUT` whose journal exceeded the 16k details bound surfaced as "details must be JSON-safe and at most 16000 characters").
- Fixed a killed CLI invocation leaving the daemon silently degraded. The daemon now aborts a running script the moment the socket that submitted it closes, so an externally killed `dev-browser` process no longer leaves an orphaned script driving pages and holding the browser lock for the rest of its timeout with nobody listening.
- Made silent output loss detectable end to end: `complete` messages for scripts carry `outputCounts`, and the CLI exits 6 with `RUNTIME_CHANNEL_LOST` when output the daemon sent never arrived, instead of exiting 0 with empty stdout.
- `doctor` now proves a real script round trip (a token echoed through the daemon's QuickJS runtime and stdout channel) whenever a compatible daemon is running, reported under `selftest` and failing with `RUNTIME_SELFTEST_FAILED` — `ok` means the execute pipeline works, not just that the socket is up.
- Sandboxed scripts on a connected Chrome now get the occluded-window wake that interactive trusted input already had (with a stderr warning when remediation ran), so lazy-loading feeds stop reading as plausibly frozen DOMs when the window is fully covered.
- Added `--version` (crate version plus a short build hash of the embedded bundles) so bug reports can carry a build identifier.
- `scroll` now rejects `--max-steps` alongside `--direction`/`--delta-y` at the CLI (previously accepted, then failed daemon-side with a contradictory cross-field error), and its error usage lists the five real modes instead of a misleading context-derived line. `examples scroll` exists.
- Documented the degraded-daemon failure signature and the recovery (`dev-browser stop` before theorizing), the hard timeout-ordering rule (CLI `--timeout` below any supervisor limit), and that plain `window.scrollBy` in a script is the normal way to drive lazy-loading feeds.
